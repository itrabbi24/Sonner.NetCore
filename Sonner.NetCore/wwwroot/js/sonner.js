/**
 * SonnerToaster - A premium, lightweight toast notification system for .NET Core.
 * @author ARG RABBI
 */
class SonnerToaster {
    /**
     * @param {Object} options Global toaster options
     * @param {string} options.position Default position ('bottom-right', 'top-center', etc.)
     * @param {boolean} options.expand Whether toasts should be expanded by default
     * @param {boolean} options.richColors Use vibrant background colors
     * @param {boolean} options.closeButton Show a close button on each toast
    ...
    /**
     * Internal method to manage toast containers for different positions
    ...
    /**
     * Returns the SVG string for the given toast type
     * @param {string} type 
     */
    getIconSvg(type) {
        switch (type) {
     ...
     * @param {string|null} position Optional position override
     */
    toast(message, type = 'Default', title = null, position = null) {
        ...
        if (this.options.closeButton) {
            closeHtml = `
                <button class="sonner-toast-close" onclick="this.parentElement.remove()" aria-label="Close">
    ...
    /**
     * Updates stacking indices for toasts in a specific position
     * @param {string} position 
     */
...
// Ensure unique global instance
if (!window.sonnerInstance) {
    window.sonnerInstance = new SonnerToaster();
}
// Backward compatibility for manual scripts
window.sonner = window.sonnerInstance;
